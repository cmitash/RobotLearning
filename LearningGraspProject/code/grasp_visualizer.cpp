    ...
    namespace packages
    {
        namespace manipulation
        {
            ...
            void grasp_visualizer_t::init(const parameter_reader_t * reader, const parameter_reader_t* template_reader)
            {
                ...
                if(boost::filesystem::exists( filename ))
                {
                    ...
                    for( i=0;i<doc.size();i++)
                    {
                        ...
                        // temp_state->at(0) = temp_vec[0];
                        // temp_state->at(1) = temp_vec[1];
                        // temp_state->at(2) = temp_vec[2];
                        // temp_state->at(3) = temp_vec[3];
                        // temp_state->at(4) = temp_vec[4];
                        // temp_state->at(5) = temp_vec[5];
                        // temp_state->at(6) = temp_vec[6];
                        ...
                        for (int i = 0; i < 99; i++) {                            
                            ...
                            // temp_state->at(0) = new_vec[0];
                            // temp_state->at(1) = new_vec[1];
                            // temp_state->at(2) = new_vec[2];
                            // temp_state->at(3) = q1[0];
                            // temp_state->at(4) = q1[1];
                            // temp_state->at(5) = q1[2];
                            // temp_state->at(6) = q1[3];
                            temp_state->at(7) = grasp_mode;
            ...
            void grasp_visualizer_t::compute_control()
            {
                ...
                unsigned index = 0;
                simulation::simulator_ptr->update_phys_configs(config_map, index);
                ...
                config_t hand_base_current;
                config_t hand_virtual_current;

                for (unsigned j = 0; j < config_map.size(); j++) {
                    std::string config_name = config_map[j].first;

                    if (config_name == "simulator/consumer/manipulator/distal_1")
                        finger_1_current = config_map[j].second;
                    else if (config_name == "simulator/consumer/manipulator/distal_2")
                        finger_2_current = config_map[j].second;
                    else if (config_name == "simulator/consumer/manipulator/distal_3")
                        finger_3_current = config_map[j].second;
                    else if (config_name == "simulator/consumer/manipulator/base")
                        hand_base_current = config_map[j].second;
                    else if (config_name == "simulator/consumer/manipulator/arm_right_reflex_virtual")
                        hand_virtual_current = config_map[j].second;                    
                }

                PRX_PRINT("Grasp ID: "<<(current_grasp_state-1)%grasping_states.size(), PRX_TEXT_BLUE);
                // PRX_PRINT("Current Base: " << hand_base_current, PRX_TEXT_GREEN);
                // PRX_PRINT("Current Virtual: " << hand_virtual_current, PRX_TEXT_GREEN);
                // PRX_PRINT("Current Finger 1: " << finger_1_current, PRX_TEXT_GREEN);
                // PRX_PRINT("Current Finger 2: " << finger_2_current, PRX_TEXT_GREEN);
                // PRX_PRINT("Current Finger 3: " << finger_3_current, PRX_TEXT_GREEN);
                ...
                if (in_collision) {
                    if (!simulation::found_end) {
                        ...
                        finger_1_config.copy(finger_1_current);
                        finger_2_config.copy(finger_2_current);
                        finger_3_config.copy(finger_3_current);
                        hand_base_config.copy(hand_base_current);
                        hand_virtual_config.copy(hand_virtual_current);
                    }
                    current_grasp_state += (100 - (current_grasp_state%100));
                ...
                if (current_grasp_state % 100 == 0) {
                    ...
                    PRX_PRINT("Hand Base End State: " << hand_base_config, PRX_TEXT_MAGENTA);
                    PRX_PRINT("Virtual Grasp End State: " << hand_virtual_config, PRX_TEXT_MAGENTA);
                    ...
                    PRX_PRINT("Finger 3 End State: " << finger_3_config, PRX_TEXT_MAGENTA);
                      ...
                      out << hand_virtual_config.get_position()[0] << "," << hand_virtual_config.get_position()[1]<<","
                        << hand_virtual_config.get_position()[2]<<","<<hand_virtual_config.get_orientation()[0]<<","
                        <<hand_virtual_config.get_orientation()[1]<<","<<hand_virtual_config.get_orientation()[2]<<","
                        <<hand_virtual_config.get_orientation()[3]<<std::endl;
