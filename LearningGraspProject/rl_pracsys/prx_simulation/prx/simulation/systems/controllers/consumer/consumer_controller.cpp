...
namespace prx
{
    ...
    namespace sim
    {
        ...
        {
            PRX_DEBUG_COLOR("Consumer controller init().", PRX_TEXT_LIGHTGRAY);
            ...
            if( !deserialize_plan.empty() )
            {
                ...
                // for(int i=0;i<4;i++)
                // {
                //     output_control_space->uniform_sample(computed_control);
                //     plan.copy_onto_back(computed_control,2.0);
                // }
                // plan.save_to_file(deserialize_plan);
                // foreach(plan_step_t& step, plan)
                // {
                //     for(int i=0;i<output_control_space->get_dimension();i++)
                //     {
                //         step.control->at(i)+=uniform_random(-.1,.1);
                //     }
                // }
            }
        ...
        void consumer_controller_t::copy_plan(const plan_t& inplan)
        {
            ...
            PRX_PRINT("Received plan", PRX_TEXT_CYAN);
            plan = inplan;
            PRX_PRINT(plan.print(10), PRX_TEXT_LIGHTGRAY);
            queried_planner = false;
