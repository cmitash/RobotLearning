...
namespace prx
{
    ...
    namespace sim
    {
        ...
        namespace simulation
        {
            ...
            simulator_t* simulator_ptr;
            bool found_end;
            util::space_t* end_state;
        ...
        simulator_t::simulator_t()
        {
            ...
            //Set the simulation time to be the beginning of time
            simulation::simulation_time.fromSec( 0.0 );
            sensing = NULL;
            simulation::simulator_ptr = this;
        ...
        void simulator_t::init(const parameter_reader_t * reader, const parameter_reader_t* template_reader)
        {
            ...
            PRX_DEBUG_COLOR(":: Collision Checker Init: " << stats_clock.measure(), PRX_TEXT_CYAN);
            stats_clock.reset();
            ...
            //     }
            // }
            PRX_DEBUG_COLOR(":: Reading obstacles: " << stats_clock.measure(), PRX_TEXT_CYAN );
            stats_clock.reset();
            router_t::init(reader, template_reader);
            PRX_DEBUG_COLOR(":: Router_t Init: " << stats_clock.measure(), PRX_TEXT_CYAN );
...
//            construct_spaces();
            ...
            PRX_DEBUG_COLOR(":: updating geoms and configs: " << stats_clock.measure(), PRX_TEXT_CYAN );
            ...
            PRX_DEBUG_COLOR(":: updating graph and getting plants: " << stats_clock.measure(), PRX_TEXT_CYAN );
            ...
            PRX_DEBUG_COLOR(":: Updating system intervals: " << stats_clock.measure(), PRX_TEXT_CYAN);

            stats_clock.reset();
            collision_checker->add_bodies(geom_map, plants);

            if(collision_checker->info_in_plants())
            {
                link_collision_info(collision_checker);
                update_collision_info();
            }
            
            collision_checker->set_configurations(config_list);
            PRX_DEBUG_COLOR(":: Collision checker setup: " << stats_clock.measure(), PRX_TEXT_CYAN);

            stats_clock.reset();
            verify();
            PRX_DEBUG_COLOR(":: simulation verification: " << stats_clock.measure(), PRX_TEXT_CYAN );
            ...
            PRX_DEBUG_COLOR(":: Initializing sensing: " << stats_clock.measure(), PRX_TEXT_CYAN);
        }
        ...
        double simulator_t::collision_distance()
        {
            ...
            {
                update_collision_info();
                return collision_checker->get_clearance();
            }   
        }
        ...
        bool simulator_t::near_collision( double eps )
        {
            ...
            {
                update_collision_info();
                return collision_checker->near_collision(eps);
            }
        }
        ...
        collision_list_t* simulator_t::get_colliding_bodies()
        {
            ...
            {
                update_collision_info();
                return collision_checker->colliding_bodies();
            }
        }
        ...
        collision_list_t* simulator_t::get_near_colliding_bodies( double eps )
        {
            ...
            {
                update_collision_info();
                return collision_checker->near_colliding_bodies(eps);
            }
        }
        ...
        void simulator_t::add_system(const std::string& path, system_ptr_t system)
        {
            ...
            system->link_collision_info(collision_checker);
        ...
        void simulator_t::update_obstacles_in_collision_checker()
        {
            ...
            {
                //do nothing since the obstacle updates collision information when getting a new root_config
            }
        }
        ...
        void simulator_t::add_obstacle(const std::string& name, system_ptr_t obstacle)
        {
            ...
            if( obstacles[name] == NULL )
            {
                ...
                if(collision_checker->info_in_plants())
                {
                    obstacle->link_collision_info(collision_checker);
                }
            }
        }
        ...
        void simulator_t::propagate(const double simulation_step)
        {
            ...
            simulation::simulation_time += ros::Duration ( simulation_step );

            router_t::propagate( simulation_step );
        }
        ...
        {
            PRX_PRINT("------------------INITIALIZE SENSING----------------", PRX_TEXT_LIGHTGRAY);
