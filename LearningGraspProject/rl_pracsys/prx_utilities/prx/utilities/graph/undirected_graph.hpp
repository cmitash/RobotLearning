        ...
        class undirected_graph_t
        {
          ...
          public:
            ...
            void remove_vertex(undirected_vertex_index_t v)
            {
                ...
                //TODO: HOW we get validity checker here to do this right?
                if(graph[v].node->constraints != NULL)
                {
                    delete graph[v].node->constraints;
                }
                graph[v].node->constraints = NULL;
            ...
            virtual void serialize(std::ofstream& output_stream, const space_t* point_space)
            {
                ...
                foreach(undirected_vertex_index_t nd, boost::vertices(graph))
                {
                    ...
                    graph[nd].node->constraints->serialize(output_stream);
                    output_stream << std::endl;
                    counter++;
                }
                counter=0;
                ...
                {
                    output_stream<<counter<<" ";
                    graph[ed].link->constraints->serialize(output_stream);
                    output_stream << std::endl;
                    counter++;
                }
            ...
            bool deserialize(const std::string& filename, std::ifstream& input_stream, const space_t* point_space)
            {
                ...
                if( input_stream.good() )
                {
                    ...
                    for( int i = 0; i < num_edges; i++ )
                    {
                        ...
                        // PRX_DEBUG_COLOR("Deserialize edge (" << from << " -> " << to << ")" , PRX_TEXT_GREEN);
