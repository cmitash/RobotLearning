...
namespace prx
{
    namespace util
    {
        ...
        double trimesh_t::get_normal_vector_for_face(int index, vector_t& normal_vector, vector_t& centroid) const
        {
            ...
            int indexA = current_face.index1;
            int indexB = current_face.index3;
            int indexC = current_face.index2;
        ...
        void trimesh_t::get_all_normal_vectors(std::vector<vector_t>& normal_vectors, std::vector<vector_t>& centroids, std::vector<double>& surface_areas) const
        {
            for (unsigned i = 0; i < faces.size(); ++i)
            {
                vector_t new_dir(3), new_centroid(3);
                surface_areas.push_back(get_normal_vector_for_face(i, new_dir, new_centroid));
