    ...
    namespace packages
    {
        namespace manipulation
        {
            ...
            bool planner_info_t::compute_solution(plan_t& plan, constraints_t* path_constraints, state_t* start_state, const std::vector< state_t* >& input_goal_states, bool ik_steer, bool set_constraints)
            {
                ...
                PRX_PRINT("===================== BEGIN COMPUTE SOLUTION =============================", PRX_TEXT_GREEN);
                ...
                if(ik_steer)
                {
                    for( unsigned i=0; i<input_goal_states.size(); ++i )
                    {
                        ...
                        bool success = manip_model->IK_steering( new_plan, result_state, start_state, goal_state, goal_config);
                        ...
                        if(success)
                        {
                            ...
                            valid_constrained_trajectory = specification->validity_checker->validate_and_generate_constraints(tmp_constraints, traj);
                            ...
                            if(valid_constrained_trajectory)
                            {
                                ...
                                manip_model->convert_plan(plan, manip_model->get_current_manipulation_info()->full_arm_control_space, new_plan, control_space);
                                //plan+=new_plan;
                                if (goal != NULL)
                                {
                                    goal->clear();
                ...
                PRX_PRINT("======================== BAD COMPUTE SOLUTION ============================", PRX_TEXT_CYAN);
                return false;
            }
        }
    }
