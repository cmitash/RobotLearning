    ...
    namespace packages
    {
        namespace manipulation
        {
            ...
            void grasping_planner_t::resolve_query()
            {
                ...
                int original_mode = manipulation_model->get_current_grasping_mode();
                original_object_state = grasping_query->object->get_state_space()->alloc_point();        
                state_t* original_manipulation_state = manipulation_model->get_state_space()->alloc_point();        
            ...
            bool grasping_planner_t::compute_plans( grasp_data_t* grasp_data, unsigned state_index, const config_t& grasping_config )
            {
                ...
                if( !( grasping_query->task_mode == TASK_PICK && !grasping_query->manipulator_retracts ) )
                {
                    ...
                    if( !manipulation_model->IK_steering( tmp_plan, grasp_data->retracted_closed_state, grasp_data->grasping_state, retraction_config ) )
                    {
                ...
                if( grasping_query->task_mode != TASK_PLACE )
                {
                    ...
                    tmp_plan.clear();
                    if( !manipulation_model->IK_steering( tmp_plan, grasp_data->retracted_open_state, grasp_data->releasing_state, retraction_config ) )
                    {
