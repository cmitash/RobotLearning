    ...
    namespace packages
    {
        ...
        namespace manipulation
        {
            ...
            bool manipulator_t::IK_steering( sim::plan_t& result_plan,  util::space_point_t* result_state, const util::space_point_t* start_state, const util::config_t& goal_config, std::string start_link, std::string end_link)
            {
                ...
                // }
                // prx_chainiksolvervel_pinv_nso ik_solver_vel(chain1,opt_pos,weights);
                // ik_solver_vel.setAlpha(.25);
                /////////////////////////////////////////////////
                ///////////////////////////////////////for regular
                ...
                result_plan.clear();
                int num_iters =0;
                int max_iters =4000*config_dist;
                ...
                while(not_done && num_iters<max_iters)
                {
                    ...
                    if(KDL::Equal(twist2,twist,1e-7))
                    {
                        completed = true;
                        not_done = false;
                        continue;                            
                    }
                    else if(KDL::Equal(twist,prev_twist,1e-7))
                    {
                        not_done = false;
                        continue;
                    }
                    ...
                    if(ik_solver_vel.CartToJnt(q_in,twist,qdot_out)>=0)
                    {
                ...
                if(!completed)
                {
                    ...
                    if(num_failures%100==0)
                        PRX_INFO_S(iter_failures<<"/"<<num_failures);
                }
